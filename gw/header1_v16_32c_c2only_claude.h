...
const int nparallel=2; // 2 threads per process; the 4 v_w cases run concurrently (see tmp_claude.sh)
...
struct F_S3hat{
  V fp;   // stored [Mc,c0,c1,c2,gam]; c2-only ansatz uses ONLY Mc=fp[0], c2=fp[3], gam=fp[4]
  ...
  double operator() (const double TmTc, const double MB) const {
    return fitter( TmTc, MB, fp[0], fp[3], fp[4] );
  }

  // c2-only thin-wall:  S3 = (MB-Mc)^gam * c2 / (T-Tc)^2      (c0 = c1 = 0)
  double fitter( const double TmTc, const double MB,
                 const double Mc,
                 const double c2,
                 const double gam ) const {
    assert(MB>Mc);
    return std::pow(MB-Mc, gam) * ( c2/TmTc/TmTc );
  }

  // dS3/d(TmTc) = (MB-Mc)^gam * ( -2 c2 / (T-Tc)^3 )
  double dT( const double TmTc, const double MB ) const {
    const double Mc = fp[0];
    const double c2 = fp[3];
    const double gam = fp[4];
    assert(MB>Mc);
    return std::pow(MB-Mc, gam) * ( -2.0 * c2/std::pow(TmTc,3) );
  }
};
...
// CLI-overridable wall velocity v (replaces the Chapman-Jouguet getv by a fixed number).
// g_v_wall < 0 means "use the physical CJ formula below"; set from the command line in main.
// When >0 it applies EVERYWHERE getv(alpha) is called: both the bubble-percolation integral
// that determines Tstar and the final vJ feeding the spectrum.
double g_v_wall = -1.0;

double getv(const double alpha){
  assert(alpha>0.0);
  if( g_v_wall > 0.0 ) return g_v_wall; // fixed wall velocity v from CLI
...
void get_gw_params( double& alpha,
                    ...
                    const int NTs = 200, // v16: widened window, keep step ~4e-6
                    const double DTc = 0.0008 // v16: widened 4e-4 -> 8e-4 (m0.4 S3 data reaches 7.8e-4)
                    ){
  Tc = f_Tc(MB);
