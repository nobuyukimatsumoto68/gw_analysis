    ...
    for(int j=0; j<nbeta; j++){
      ...
      for(std::string basedir2 : std::vector<std::string>{basedir2c, basedir2h} ){
        for(int conf=conf_min0; conf<conf_max0; conf++){
          ...
          // sig0p5: read the halved-sigma avgPr_hist from base_dir (argv[2]=32_v15), NOT basedir2.
          // op is still read from basedir2c (32_v9) below; basedir2c==basedir2h keeps the type gate.
          // base_dir is otherwise unused, so this only redirects the avgPr_hist source -> the run is
          // bit-for-bit the production reweight with the halved-sigma histograms swapped in.
          // ORIGINAL: std::string pathO3 = basedir2+obs_id+"avgPr_hist"+std::to_string(conf)+".bin";
          std::string pathO3 = base_dir+obs_id+"avgPr_hist"+std::to_string(conf)+".bin";
...
#pragma omp parallel for num_threads(threads) collapse(2)
    for(int ibx=0; ibx<nbins_histo_x; ibx++){ // histogram binning for
      // real-axis only (Claude variant): the analysis get_Gamma (header.h) reads ONLY the
      // two central iby rows -- irow1 = nbins_histo_y/2-1 and its mirror nbins_histo_y/2
      // (Im(P) \approx 0, the real axis). Reweighting just these two rows avoids the
      // expensive ReweightedObs on the ~nbins_histo_y-2 off-axis rows that are never read.
      // Original: for(int iby=0; iby<nbins_histo_y; iby++){
      for(int iby=nbins_histo_y/2-1; iby<=nbins_histo_y/2; iby++){ // histogram binning for

        // v16 FIX: GridLogMessage touches a shared GridStopWatch timer; called from inside the
        // `omp parallel for` (line ~179) by multiple threads it races -> "Timer.h: running==true"
        // GRID_ASSERT -> MPI_ABORT (crashed at OMP=8). Use a plain std::cout in an omp critical
        // (no Grid timer) -- progress print only, no functional change.
#pragma omp critical
        std::cout << "ibx = " << ibx << " iby = " << iby << std::endl;
