...
#pragma omp parallel for num_threads(threads)
      // real-axis only (Claude variant): the analysis get_Gamma reads ONLY the two central
      // iby rows (irow1 = nbins_histo_y/2-1 and its mirror nbins_histo_y/2; Im(P) \approx 0).
      // Reweighting just these two rows skips the never-read off-axis reweightings.
